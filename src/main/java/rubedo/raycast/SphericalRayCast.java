		...
		for (int x = 0; x < this.boundSize; ++x)
			for (int y = 0; y < this.boundSize; ++y)
				for (int z = 0; z < this.boundSize; ++z) {
					...
					if (x == 0 || x == this.boundSize - 1 || y == 0
							|| y == this.boundSize - 1 || z == 0
							|| z == this.boundSize - 1) {
						// Clamp to [-1,1]
						...
						Vec3 direction = ShapedRayCast.normalizeVector(
								this.world, xCoord, yCoord, zCoord);

						castBlockRay(direction.xCoord, direction.yCoord,
								direction.zCoord, filter, output);
					}
				}
	...
	public Set<Entity> getEntitiesExcludingEntity(Entity excludedEntity,
		...
		List<Entity> entities = this.world
				.getEntitiesWithinAABBExcludingEntity(excludedEntity,
						AxisAlignedBB
								.getBoundingBox(this.originX - this.range,
										this.originY - this.range, this.originZ
												- this.range, this.originX
												+ this.range, this.originY
												+ this.range, this.originZ
												+ this.range), selector);
