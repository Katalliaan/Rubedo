...
public abstract class ShapedRayCast implements IShapedRayCast {
	/**
	 * When creating the ray targets, this decides the size of the shape structure.
	 ...
	 */
	protected void castBlockRay(
			double directionRayX, double directionRayY, double directionRayZ, 
			IBlockRayFilter filter, Collection<ChunkPosition> collection) {
		...
		for (float length = 0.0F; length <= actualRange; length += stepSize) {
			int blockX = MathHelper.floor_double(this.originX + (directionRayX * length));
