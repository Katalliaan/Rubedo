...
public class EntityAIGroundEater extends EntityAIBase {
	...
	public boolean shouldExecute() {
		return entity.isEntityAlive() && !entity.isInLove()
				&& entity.getGrowingAge() == 0;
	}
	...
	public boolean continueExecuting() {
		if (entity.isEntityAlive() && !entity.isInLove()
				&& entity.getGrowingAge() == 0) {
			@SuppressWarnings("rawtypes")
			List nearbyItems = entity.worldObj
					.getEntitiesWithinAABBExcludingEntity(entity,
							entity.boundingBox.expand(1.0D, 0.0D, 1.0D));
			...
			if (nearbyItems != null && nearbyItems.size() > 0) {
				for (int itemId = 0; itemId < nearbyItems.size(); itemId++) {
					...
					if ((e instanceof EntityItem)
							&& entity.isBreedingItem(((EntityItem) e)
									.getEntityItem())) {
						return true;
					}
				}
	...
	public void startExecuting() {
		if (entity.isEntityAlive() && !entity.isInLove()
			...
			List nearbyItems = entity.worldObj
					.getEntitiesWithinAABBExcludingEntity(entity,
							entity.boundingBox.expand(2.0D, 0.0D, 2.0D));
			if (nearbyItems != null && nearbyItems.size() > 0) {
				for (int itemId = 0; itemId < nearbyItems.size(); itemId++) {
					Entity e = (Entity) nearbyItems.get(itemId);
					if ((e instanceof EntityItem)
						...
						if (((EntityItem) e).getEntityItem().stackSize >= 1) {
							...
							entity.worldObj
									.playSoundAtEntity(
											...
											"random.pop",
											0.2F,
											((entity.getRNG().nextFloat() - entity
